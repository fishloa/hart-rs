...
impl CommandResponse for ReadLongTagResponse {
    ...
    fn decode_data(data: &[u8]) -> Result<Self, DecodeError> {
        ...
        Ok(ReadLongTagResponse { long_tag })
    }
}
...
mod tests {
    ...
    fn test_cmd20_command_number() {
        assert_eq!(ReadLongTagRequest::COMMAND_NUMBER, 20);
        assert_eq!(ReadLongTagResponse::COMMAND_NUMBER, 20);
    }

    #[test]
    fn test_cmd20_request_encodes_no_data() {
        let req = ReadLongTagRequest;
        let mut buf = [0u8; 4];
    ...
    fn test_cmd20_response_decode() {
        ...
        let resp = ReadLongTagResponse::decode_data(&data).unwrap();
    ...
    fn test_cmd20_response_too_short() {
        ...
        assert_eq!(
            ReadLongTagResponse::decode_data(&data),
