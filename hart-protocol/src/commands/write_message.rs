...
pub struct Cmd17Response {
    pub message: [u8; 32],
}

impl CommandRequest for Cmd17Request {
...
impl CommandResponse for Cmd17Response {
    const COMMAND_NUMBER: u8 = WRITE_MESSAGE;
...
mod tests {
    ...
    fn test_cmd17_command_number() {
        assert_eq!(Cmd17Request::COMMAND_NUMBER, 17);
        assert_eq!(Cmd17Response::COMMAND_NUMBER, 17);
    }
    ...
    fn test_cmd17_request_encode_and_response_decode_roundtrip() {
        ...
        let req = Cmd17Request { message: msg };

        let mut buf = [0u8; 24];
        let len = req.encode_data(&mut buf).unwrap();
        assert_eq!(len, 24);

        let resp = Cmd17Response::decode_data(&buf[..len]).unwrap();
    ...
    fn test_cmd17_response_too_short() {
        ...
        assert_eq!(
            Cmd17Response::decode_data(&data),
